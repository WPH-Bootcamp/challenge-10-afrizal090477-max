...
export default function CheckoutPage() {
  ...
  // 💡 INITIAL STATE NULL: Tidak mengunci di BNI secara default, murni kosong di awal
  const [selectedBank, setSelectedBank] = useState<string | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  // Ambil data murni dari interface global API tanpa bypass 'any'
  ...
  // Ambil nama bank aktif jika ada yang dipilih
  const activeBankName = BANKS.find((b) => b.id === selectedBank)?.name || "";

  const handleBuyProcess = () => {
    if (foodItems.length === 0) return;
    
    // Validasi: Jika user belum memilih bank, jangan buka modal sukses
    if (!selectedBank) {
      ...
      <div className="w-full max-w-[1000px] mx-auto px-4 md:px-0 flex flex-col gap-[24px]">
        ...
        {!isPending && !isError && foodItems.length > 0 && (
          <div className="w-full flex flex-col lg:flex-row items-start gap-[20px]">
            ...
            {/* GRID KIRI: Detail Alamat & Menu Pembelian */}
            <div className="w-full lg:w-[590px] flex flex-col gap-[20px]">
              
              {/* Box 1: Delivery Address Card */}
              <div 
            ...
            {/* GRID KANAN: Bank Radio SVG & Payment Summary */}
            <div 
            ...
            >
              <div className="w-full flex flex-col gap-[16px] px-[20px]">
                ...
                <div className="flex flex-col w-full gap-[12px]">
                  {BANKS.map((bank) => {
                    // 💡 MURNI DINAMIS: Hanya true jika selectedBank sama dengan bank.id saat di-klik
