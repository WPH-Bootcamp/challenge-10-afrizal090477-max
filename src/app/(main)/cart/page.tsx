...
import { useGetCart, useAddToCart, useDeleteFromCart } from "@/lib/query/useCart"; 
import { ChevronRight } from "lucide-react";
import { CartSummary } from "@/components/features/cart/CartComponents";
                ...
                >
                  ...
                  >
                    <div className="relative w-[32px] h-[32px] rounded-[4px] overflow-hidden bg-transparent flex-shrink-0">
                      <Image src={restaurantLogo} alt="Icon Resto" fill sizes="32px" className="object-contain" />
                    </div>
                    <h3 className="text-[18px] font-[700] text-[#0A0D12] truncate max-w-[150px]">{restaurantName}</h3>
                  ...
                  <div className="flex flex-col w-full">
                    {group.items.map((item: CartItem) => (
                      <CartItemCard 
                        ...
                        isMutating={updateCartMutation.isPending || deleteCartMutation.isPending} 
                      />
                    ))}
                  </div>
